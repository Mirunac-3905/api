...
app.get('/api/search', async (req, res) => {
    ...
    try {
        ...
        if (query) {
            filter.$text = { $search: query };  // Use MongoDB's text search
        }
...
const SubscriptionSchema = new mongoose.Schema({
  ...
  // First transporter configuration (original email)
const transporter1 = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: 'miruna392005@gmail.com',
      pass: 'hbbf cqgh czaf dasb',
    },
  });
  ...
  // Send notification email function for both configurations
  const sendNotificationEmail = (email, transporter) => {
    const mailOptions = {
      from:"miruna392005@gmail.com",  // Sender email
      to: email,  // Send email to the user who subscribed
  ...
  app.post('/subscribe', async (req, res) => {
    ...
    try {
      ...
      // Send email using the first transporter
      sendNotificationEmail(email, transporter1);  // Or use transporter2 for the second email configuration
